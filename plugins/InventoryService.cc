...
Task<std::vector<InventoryItem>> InventoryService::items(){co_return co_await repo_.listItems();}
...
Task<std::vector<InventoryLot>> InventoryService::lots(){co_return co_await repo_.listLots();}
...
Task<std::vector<RecipeLink>> InventoryService::recipes(std::optional<int64_t>p){co_return co_await repo_.listRecipes(p);}
